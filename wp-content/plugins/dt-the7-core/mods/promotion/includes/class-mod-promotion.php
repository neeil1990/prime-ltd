<?php
/**
 * Promotion module main class.
 */

// File Security Check
if ( ! defined( 'ABSPATH' ) ) { exit; }

if ( ! class_exists( 'Presscore_Mod_Promotion', false ) ) {

	class Presscore_Mod_Promotion {
		...
		private function load_dependencies() {
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-mod-promotion-admin.php';

			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-mod-promotion-public.php';
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-mod-promotion-scroller.php';
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/class-mod-promotion-ajax-content-builder.php';
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/mod-promotion-helpers.php';
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/mod-promotion-template-config-helpers.php';
			require_once plugin_dir_path( dirname( __FILE__ ) ) . 'public/widgets/promotion.php';
		}

		private function setup_services() {
			presscore_template_manager()->add_path( 'mod_promotion', array( 'mods/promotion/public/templates', 'inc/mods/promotion/public/templates' ) );
		}

		private function define_admin_hooks() {
			// rewrite post type slug
			$rewrite_filter = new Presscore_Post_Type_Rewrite_Rules_Filter( 'general-post_type_promotion_slug' );

			// filter post type args prior it's registration
			add_filter( 'presscore_post_type_dt_promotion_args', array( $rewrite_filter, 'filter_post_type_rewrite' ) );
			...
			$mod_admin = new Presscore_Mod_Promotion_Admin();
			...
			add_filter( 'manage_edit-dt_promotion_columns', 'presscore_admin_add_thumbnail_column' );

			// add sidebars columns
			add_filter( 'manage_edit-dt_promotion_columns', 'presscore_admin_add_sidebars_columns' );
		}

		private function define_public_hooks() {
			$mod_public = new Presscore_Mod_Promotion_Public();
			...
			add_filter( 'presscore_archive_post_content-dt_promotion', array( $mod_public, 'archive_post_content' ) );
