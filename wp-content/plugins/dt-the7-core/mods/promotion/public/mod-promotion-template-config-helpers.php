<?php
/**
 * Promotion template config helpers.
 */
...
if ( ! function_exists( 'presscore_congif_populate_promotion_vars' ) ) :

	function presscore_congif_populate_promotion_vars() {
		...
		if ( ! $config->get( 'display' ) ) {
			$display = get_post_meta( $post_id, '_dt_promotion_display', true );
		...
		switch ( $template_name ) {
			...
			case 'template-promotion-masonry.php' :
				$config->map( array(
			...
			case 'template-promotion-jgrid.php':
				$config->map( array(
...
if ( ! function_exists( 'presscore_congif_populate_single_promotion_vars' ) ) :

	function presscore_congif_populate_single_promotion_vars() {
		...
		// general meta switch
		if ( of_get_option( 'general-promotion_meta_on', 1 ) ) {

			// date
			$config->set( 'post.meta.fields.date', of_get_option( 'general-promotion_meta_date', 1 ) );

			// categories
			$config->set( 'post.meta.fields.categories', of_get_option( 'general-promotion_meta_categories', 1 ) );

			// comments
			$config->set( 'post.meta.fields.comments', of_get_option( 'general-promotion_meta_comments', 1 ) );

			// author
			$config->set( 'post.meta.fields.author', of_get_option( 'general-promotion_meta_author', 1 ) );
		...
		$config->set( 'post.navigation.arrows.enabled', of_get_option( 'general-next_prev_in_promotion', 1 ) );
...
if ( ! function_exists( 'presscore_populate_promotion_config' ) ) :

	function presscore_populate_promotion_config( $target_post_id = 0 ) {
