...
if ( ! class_exists( 'DT_Shortcode_Promotion_Slider', false ) ) {

	class DT_Shortcode_Promotion_Slider extends DT_Shortcode {

		protected $shortcode_name = 'dt_promotion_slider';
		protected $post_type = 'dt_promotion';
		protected $taxonomy = 'dt_promotion_category';
		...
		public function shortcode( $atts, $content = null ) {
			...
			if ( presscore_vc_is_inline() ) {
				...
				return $this->vc_inline_dummy( array(
					'class' => 'dt_vc-promotion_scroller',
					'title' => _x( 'Promotion scroller', 'vc inline dummy', 'the7mk2' ),
					'fields' => array( $terms_title => $terms_list )
				) );
			}

			return $this->promotion_slider();
		}

		protected function promotion_slider() {
			$output = '';
			...
			if ( $dt_query->have_posts() ) {
				...
				// shape output
				$output = '<div ' . $this->get_container_html_class( array( 'dt-promotion-shortcode', 'slider-wrapper' ) ) . ' ' . $this->get_container_data_atts() . '>';
		...
		protected function setup_config() {
			...
			$config->map( array(
				'template'								=> array( 'value', 'promotion' ),
		...
		protected function add_hooks() {
			add_filter( 'dt_promotion_thumbnail_args', array( &$this, 'set_image_dimensions' ) );
		}

		protected function remove_hooks() {
			remove_filter( 'dt_promotion_thumbnail_args', array( &$this, 'set_image_dimensions' ) );
		}

	}

	add_shortcode( 'dt_promotion_slider', array( new DT_Shortcode_Promotion_Slider(), 'shortcode' ) );
}
