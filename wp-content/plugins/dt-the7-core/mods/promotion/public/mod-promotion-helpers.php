 ...
 * Promotion helpers.
 *
 * @package the7\Promotion\Helpers
 * @since 3.0.0
 */
	...
	function presscore_display_related_projects() {
		...
		if ( $config->get( 'post.related_posts.enabled' ) ) {
			...
			switch ( $config->get( 'post.related_posts.query.mode' ) ) {
				...
				default: $terms = wp_get_object_terms( $post->ID, 'dt_promotion_category', array('fields' => 'ids') );
			}
			...
			if ( $terms && !is_wp_error( $terms ) ) {
				...
				$promotion_scroller = new Presscore_Promotion_Posts_Scroller();
				$promotion_scroller->setup( $posts, array(
				...
				$html .= $promotion_scroller->get_html();
...
if ( ! function_exists( 'presscore_get_project_media_slider' ) ) :
	...
	function presscore_get_project_media_slider( $class = array() ) {
		...
		if ( count( $attachments_data ) > 1 ) {
			...
			$thumb_args = apply_filters( 'dt_promotion_thumbnail_args', $thumb_args );
