...
class Presscore_Inc_Widgets_Promotion extends WP_Widget {
	...
	function __construct() {  
		/* Widget settings. */
		$widget_ops = array( 'description' => _x( 'Promotion projects', 'widget', 'the7mk2' ) );

		/* Create the widget. */
		parent::__construct(
			'presscore-promotion',
			DT_WIDGET_PREFIX . _x( 'Promotion projects', 'widget', 'the7mk2' ),
			$widget_ops
		);
	}
	...
	function form( $instance ) {
		...
		$terms = get_terms( 'dt_promotion_category', array(
			'hide_empty'    => 1,
			'hierarchical'  => false 
		) );
